...
class SettingsDialog(QDialog):
    ...
    def __init__(self, config: dict, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QTabWidget::pane {
                ...
                background-color: rgba(18, 20, 32, 0.82);
                border-radius: 8px;
                padding: 4px;
            }
            QTabBar::tab {
            ...
            QTabBar::tab:hover:!selected {
                background-color: rgba(45, 48, 70, 0.95);
                color: #FFFFFF;
            }
            QScrollArea {
                background: transparent;
    ...
    def paintEvent(self, event):
        ...
        if self.bg_pixmap and not self.bg_pixmap.isNull():
            ...
            painter.fillRect(self.rect(), QColor(14, 16, 26, 215))
        else:
    ...
    def _build_ui(self):
        ...
        subtitle_label = QLabel("Spotify 浮動歌詞オーバーレイ • Floating Lyrics Settings")
        subtitle_label.setStyleSheet("font-size: 8.5pt; color: #FFB7C5; font-weight: 500;")
        ...
        self.tabs.addTab(self._create_language_tab(), "🌐 Language & Script")
        self.tabs.addTab(self._create_gaming_tab(), "🎮 Gaming & Effects")
        main_layout.addWidget(self.tabs)

        # Persistent Footer Buttons (Always visible on any screen resolution!)
    ...
    def _create_appearance_tab(self) -> QWidget:
        ...
        # 1. Position Card
        pos_box = QFrame()
        pos_box.setStyleSheet(card_style)
        ...
        lbl_pos_title.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9.5pt;")
        ...
        # 2. Width & Font Size Card
        size_box = QFrame()
        size_box.setStyleSheet(card_style)
        size_layout = QVBoxLayout(size_box)
        size_layout.setContentsMargins(12, 8, 12, 8)
        size_layout.setSpacing(5)

        self.lbl_width_val = QLabel("Overlay Width: 1400 px")
        self.lbl_width_val.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9pt;")
        ...
        self.lbl_size_val.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9pt;")
        ...
        # 3. Active Lyric Color Card
        color_box = QFrame()
        color_box.setStyleSheet(card_style)
        color_layout = QHBoxLayout(color_box)
        color_layout.setContentsMargins(12, 8, 12, 8)
        color_layout.setSpacing(8)

        lbl_color = QLabel("Color:")
        lbl_color.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9pt;")
        ...
        # 4. Lock Checkbox Card
        chk_box = QFrame()
        chk_box.setStyleSheet(card_style)
        chk_layout = QVBoxLayout(chk_box)
        chk_layout.setContentsMargins(12, 6, 12, 6)

        self.chk_lock = QCheckBox("Lock overlay position (prevent accidental mouse dragging)")
    ...
    def _create_language_tab(self) -> QWidget:
        ...
        # 1. Language Script Card
        lang_box = QFrame()
        lang_box.setStyleSheet(card_style)
        lang_layout = QVBoxLayout(lang_box)
        lang_layout.setContentsMargins(12, 8, 12, 8)
        lang_layout.setSpacing(4)

        lbl_lang_title = QLabel("🌐 Lyrics Script / Language Mode")
        lbl_lang_title.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9.5pt;")
        ...
        ctx_box = QFrame()
        ctx_box.setStyleSheet(card_style)
        ...
        lbl_ctx_title.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9.5pt;")
    ...
    def _create_gaming_tab(self) -> QWidget:
        container = QWidget()
        layout = QVBoxLayout(container)
        layout.setContentsMargins(10, 10, 10, 10)
        layout.setSpacing(8)
        ...
        # 1. Gaming Mode & Click-Through Card
        game_box = QFrame()
        game_box.setStyleSheet(card_style)
        game_layout = QVBoxLayout(game_box)
        game_layout.setContentsMargins(12, 8, 12, 8)
        game_layout.setSpacing(4)

        lbl_game_title = QLabel("🎮 Gaming & Fullscreen Overlay")
        lbl_game_title.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9.5pt;")
        game_layout.addWidget(lbl_game_title)

        self.chk_click_through = QCheckBox("Game Mode: Pass clicks through overlay to game (Click-Through)")
        self.chk_click_through.setStyleSheet("color: #FFFFFF; font-size: 9pt;")
        game_layout.addWidget(self.chk_click_through)

        lbl_game_tip = QLabel("💡 Tip: Set your game's video mode to 'Borderless Windowed' or 'Windowed Fullscreen' so desktop overlays can display on top.")
        ...
        # 2. Transition Effect Card
        trans_box = QFrame()
        trans_box.setStyleSheet(card_style)
        trans_layout = QVBoxLayout(trans_box)
        trans_layout.setContentsMargins(12, 8, 12, 8)
        trans_layout.setSpacing(3)

        lbl_trans_title = QLabel("✨ Lyric Transition Animation")
        lbl_trans_title.setStyleSheet("font-weight: bold; color: #FFB7C5; font-size: 9.5pt;")
        trans_layout.addWidget(lbl_trans_title)

        self.radio_trans_float = QRadioButton("Smooth Float Up (Lyrics glide upwards smoothly)")
        self.radio_trans_instant = QRadioButton("Instant Pop (Instant text switch without motion)")
        ...
        badge.setStyleSheet("font-weight: bold; color: #1DB954; font-size: 9pt;")
        status_layout.addWidget(badge)

        desc = QLabel("Captures playback automatically. No login needed!")
        desc.setStyleSheet("color: #B5B8CD; font-size: 8pt;")
    ...
    def _on_width_slider_changed(self, val: int):
        self.lbl_width_val.setText(f"Overlay Width: {val} px")

    def _on_size_slider_changed(self, val: int):
        self.lbl_size_val.setText(f"Font Size: {val} pt")

    def _choose_color(self):
        col = QColorDialog.getColor(QColor(self.current_color), self, "Choose Active Lyric Color")
        if col.isValid():
            self.current_color = col.name().upper()
            self._update_color_preview_btn()

    def _set_color_preset(self, hex_val: str):
        self.current_color = hex_val.upper()
        self._update_color_preview_btn()
